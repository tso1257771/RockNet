...
outpath = os.path.join(basepath, 
    'tfrecord_SingleSta', 'RF_multiple_sta_label', 'train')
outfigpath = os.path.join(basepath, 
    'fig_tfrecord_SingleSta', 'RF_multiple_sta_label', 'train')
...
rf_train_evid = partition_data.item()['train']
...
for i in range(len(rf_train_evid)):
    ...
    if os.path.exists(os.path.join(wfdir, evid)):
        logging.info(f"Generating training data: {rf_train_evid[i]}")
    else:
        continue

...
outpath = os.path.join(basepath, 
    'tfrecord_SingleSta', 'RF_multiple_sta_label', 'test')
outfigpath = os.path.join(basepath, 
    'fig_tfrecord_SingleSta', 'RF_multiple_sta_label', 'test')
...
rf_test_evid = partition_data.item()['test']
...
train_ct = []
for i in range(len(rf_test_evid)):
    evid = str(rf_test_evid[i])
    if os.path.exists(os.path.join(wfdir, evid)):
        logging.info(f"Generating test data: {rf_test_evid[i]}")
    else:
        continue
    ...
    for j in range(len(stas)):
        ...
        seg_start_sec = np.array(collect_start_sec)

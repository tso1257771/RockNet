...
from .data_utils import gen_tar_func, gen_tar_func_triangle
from .data_utils import MWA_joint_ENZsnr, MWA_suffix_Psnr

def mosaic_basic_info(data_path, ev_pair,
            env_len_pair, wf_def='ps_res',
            base_wf_sec=20, secs_bef_P=0.5, dt=0.01, 
            use_aicP=True, use_aicS=True, data_type='TW'):
    p_utc = []
    residual_ps = []
    trc_pairs = []
    env_len_pairs = []
    for t in range(len(ev_pair)):
        if data_type == 'TW':
            if  np.logical_or(
                    ev_pair[t].split('/')[6] == 'TW_aicPS_60s_ps10_up',
                    ev_pair[t].split('/')[6] == 'TW_aicPS_30s_ps10_up'):
                subdir = 'TW_PS_10s/' + '/'.join(ev_pair[t].split('/')[7:9])
                m_type = '.'.join(ev_pair[t].split('/')[9].split('.')[3:5])
            elif np.logical_or(
                    ev_pair[t].split('/')[6] == 'TW_aicPS_60s_ps10_below',
                    ev_pair[t].split('/')[6] == 'TW_aicPS_30s_ps10_below'):
                subdir = '/'.join(ev_pair[t].split('/')[7:10])
                m_type = '.'.join(ev_pair[t].split('/')[10].split('.')[3:5])
        elif data_type == 'SC':
            subdir = ev_pair[t].split('/')[7]
            m_type = '.'.join(ev_pair[t].split('/')[8].split('.')[1:4])            
        try:
            # waveform didn't correctly named with respect 
            # to its recorded channel information
            if data_type == 'TW':
                trc = read(os.path.join(data_path, subdir, 
                            f"*.{m_type}.*.sac"))
            if data_type == 'SC':    
                trc = read(os.path.join(data_path, subdir, 
                            f"{m_type}.*.sac"))
        ...
        info = trc[0].stats
        if use_aicP: 
            tp = info.starttime-info.sac.b+info.sac.t5
        elif not use_aicP: 
            tp = info.starttime-info.sac.b+info.sac.t1
        if use_aicP:
            ts = info.starttime-info.sac.b+info.sac.t6
        elif not use_aicS: 
            tp = info.starttime-info.sac.b+info.sac.t2
        residual = ts-tp
        ...
        if wf_def=='envelope':
            env_len_pairs.append(env_len_pair[t])
        
    if len(trc_pairs) != len(ev_pair):
        return None, None, None, None, None, None

    p_utc = np.array(p_utc)
    s_utc = p_utc + residual_ps
    init_stt = np.array(p_utc) - secs_bef_P
    env_len_pairs = np.array(env_len_pairs)
    residual_ps = np.array(residual_ps)
    #if np.any(env_len_pairs - 2*residual_ps) < 0:
    #    replace_idx = np.where ((env_len_pairs - 2*residual_ps) < 0)[0]
    #    env_len_pairs[replace_idx] = 2*residual_ps[replace_idx]

    if wf_def == 'ps_res':
        #base_period = np.round(np.sum(residual_ps)*2, 2) +\
        #            np.round(len(residual_ps)*secs_bef_P, 2)
        #init_ent = np.array(p_utc) + \
        #            2*np.array(residual_ps)
        base_period = np.round(np.sum(residual_ps)*1.5, 2) +\
                    np.round(len(residual_ps)*secs_bef_P, 2)
        init_ent = np.array(p_utc) + \
                    1.5*np.array(residual_ps)
    elif wf_def == 'envelope':
        base_period = np.round(np.sum(env_len_pairs), 2) +\
                np.round(len(env_len_pairs)*secs_bef_P, 2)
        init_ent = np.array(p_utc) + np.array(env_len_pairs)
...
def mosaic_wf_marching(base_wf_sec,
    ...
    new_s_utc = []
    new_mosaic_utc = []
    base_trc = trc_pairs[0].slice(
        slice_idx_to_end[0][0], slice_idx_to_end[0][1])
    
    #if scaling:
    #    scaling = np.random.uniform(scaling[0], scaling[1])
    #    for b in base_trc:
    #        b.data = b.data*scaling
    ...
    for ct, p in enumerate(slice_idx_to_end):
        if ct == 0:
            ...
            mosaic_trc = trc_pairs[ct].slice(p[0], p[1])
    ...
    for ms in range(len(mosaic_stt)):
        ...
        ms_wf = base_trc.copy().slice(ms_stt, ms_ent)
        ms_wf.sort()
    ...
    joint_ENZsnrs_pairs = MWA_joint_ENZsnr(trc_mosaic_pair[1],
         joint_pt_pair[1], snr_win=1, dt=0.01, 
         hpfreq=2, mode='std')

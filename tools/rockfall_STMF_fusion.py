...
class RockFall_STMF:
    ...
    def predict(self, wf=None, postprocess=False):
        ...
        wf_slices, spectrogram, pad_bef, pad_aft = conti_standard_wf_fast(wf, 
            pred_npts=self.pred_npts, 
            pred_interval_sec=self.pred_interval_sec, dt=self.dt)
        ## make prediction
        t1 = time()
        eqpick, eqmask, rfmask = self.model.predict([wf_slices, spectrogram])
        ...
        array_P_med, array_S_med, array_eqM_med, array_rfM_med = \
            pred_MedianFilter(
                eqpick=eqpick, eqmask=eqmask,
                rfmask=rfmask, wf_npts=wf_npts, 

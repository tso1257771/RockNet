...
def fusion_unet(pretrained_weights=None):
    ...
    #att = MultiHeadAttention(num_heads=8, key_dim=ts_exp_Es[-1].shape[1])(
    #    fusion,  ts_exp_Es[-1])
    ...
    # Decoder
    spec_Ds = []
    for i in range(len(spec_nb_filters)):
        if i == 0:
            spec_D_fus = spec_RRconv_unit(
                inputs=spec_Es[-1],
                spec_nb_filter=spec_nb_filters[-1],
                spec_kernel_size=spec_kernel_size,
                kernel_regu=kernel_regu,
                spec_strides=None,
                name=f"spec_D{i+1}"
            ...
            spec_D = spec_upconv_unit(
                inputs=spec_Ds[-1],
                spec_nb_filter=spec_nb_filters[-1-i],
                concatenate_layer=spec_Es[-1-i],
                spec_kernel_size=spec_kernel_size,
                kernel_regu=kernel_regu,
...
def fusion_unet_1task(pretrained_weights=None):
    #################### build time-frequency model ####################
    # ========== Encoder
    spec_Es = []
    # initialize

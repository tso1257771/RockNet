import tensorflow as tf
import numpy as np
'''
Sources:
https://www.pyimagesearch.com/2020/03/23/using-tensorflow-and-gradienttape-to-train-a-keras-model/

# Encapsulating the forward and backward passs of data using
# tf.GradientTape for updating model weights.

'''
#--  distributed training
@tf.function
def distributed_train_step_RF(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_ps, train_EQmask, train_RFmask, 
        loss_estimator, class_weight):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_ps, train_pred_EQmask, \
                train_pred_RFmask = model(train_trc, training=True)
            per_replica_losses_ps = \
                loss_estimator(train_ps, train_pred_ps)
            per_replica_losses_EQmask = \
                loss_estimator(train_EQmask, train_pred_EQmask)
            per_replica_losses_RFmask = \
                loss_estimator(train_RFmask, train_pred_RFmask)

            per_replica_losses = \
                per_replica_losses_ps * class_weight[0] +\
                per_replica_losses_EQmask * class_weight[1] + \
                per_replica_losses_RFmask * class_weight[2]
            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses, per_replica_losses_ps, \
            per_replica_losses_EQmask, per_replica_losses_RFmask

    per_replica_losses, per_replica_losses_ps, \
        per_replica_losses_EQmask, per_replica_losses_RFmask = \
        strategy.run(
            train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )                 
    mean_batch_loss_ps = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_ps, axis=None )
    mean_batch_loss_EQmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_EQmask, axis=None ) 
    mean_batch_loss_RFmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_RFmask, axis=None ) 
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss, mean_batch_loss_ps, \
        mean_batch_loss_EQmask, mean_batch_loss_RFmask

@tf.function
def distributed_val_step_RF(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_ps, val_EQmask, val_RFmask,
        loss_estimator, class_weight):
        # estimate validation data 
        val_pred_ps, val_pred_EQmask, val_pred_RFmask =\
            model(val_trc, training=False)
        per_replica_losses_ps = loss_estimator(val_ps, val_pred_ps)
        per_replica_losses_EQmask = loss_estimator(val_EQmask, val_pred_EQmask)
        per_replica_losses_RFmask = loss_estimator(val_RFmask, val_pred_RFmask)

        per_replica_losses = \
            per_replica_losses_ps*class_weight[0] +\
            per_replica_losses_EQmask*class_weight[1] +\
            per_replica_losses_RFmask*class_weight[2]

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss

@tf.function
def distributed_train_step_RF_fusion(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_spec, train_ps, train_EQmask, train_RFmask, 
        loss_estimator, class_weight):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_ps, train_pred_EQmask, \
                train_pred_RFmask = model([train_trc, train_spec], training=True)
            per_replica_losses_ps = \
                loss_estimator(train_ps, train_pred_ps)
            per_replica_losses_EQmask = \
                loss_estimator(train_EQmask, train_pred_EQmask)
            per_replica_losses_RFmask = \
                loss_estimator(train_RFmask, train_pred_RFmask)

            per_replica_losses = \
                per_replica_losses_ps * class_weight[0] +\
                per_replica_losses_EQmask * class_weight[1] + \
                per_replica_losses_RFmask * class_weight[2]
            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses, per_replica_losses_ps, \
            per_replica_losses_EQmask, per_replica_losses_RFmask

    per_replica_losses, per_replica_losses_ps, \
        per_replica_losses_EQmask, per_replica_losses_RFmask = \
        strategy.run(
            train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )                 
    mean_batch_loss_ps = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_ps, axis=None )
    mean_batch_loss_EQmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_EQmask, axis=None ) 
    mean_batch_loss_RFmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_RFmask, axis=None ) 
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss, mean_batch_loss_ps, \
        mean_batch_loss_EQmask, mean_batch_loss_RFmask

@tf.function
def distributed_val_step_RF_fusion(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_spec, val_ps, val_EQmask, val_RFmask,
        loss_estimator, class_weight):
        # estimate validation data 
        val_pred_ps, val_pred_EQmask, val_pred_RFmask =\
            model([val_trc, val_spec], training=False)
        per_replica_losses_ps = loss_estimator(val_ps, val_pred_ps)
        per_replica_losses_EQmask = loss_estimator(val_EQmask, val_pred_EQmask)
        per_replica_losses_RFmask = loss_estimator(val_RFmask, val_pred_RFmask)

        per_replica_losses = \
            per_replica_losses_ps*class_weight[0] +\
            per_replica_losses_EQmask*class_weight[1] +\
            per_replica_losses_RFmask*class_weight[2]

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss

@tf.function
def distributed_train_step_RF_fusion_net(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_spec, train_ps, train_EQmask, train_RFmask,
        train_eqocc, train_rfocc,
        loss_estimator, class_weight):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_rfocc, train_pred_eqocc, train_pred_ps,\
                train_pred_EQmask, train_pred_RFmask = model(
                    [train_trc, train_spec], training=True
                )
            #per_replica_losses_eqocc = \
            #        loss_estimator(train_eqocc, train_pred_eqocc)
            per_replica_losses_rfocc = \
                    loss_estimator(train_rfocc, train_pred_rfocc)
            per_replica_losses_ps = \
                tf.reduce_sum(
                    loss_estimator(train_ps, train_pred_ps),
                axis=1)
            per_replica_losses_EQmask = \
                tf.reduce_sum(
                    loss_estimator(train_EQmask, train_pred_EQmask),
                axis=1)
            per_replica_losses_RFmask = \
                tf.reduce_sum(
                    loss_estimator(train_RFmask, train_pred_RFmask),
                axis=1)

            per_replica_losses =\
                per_replica_losses_rfocc * class_weight[0] +\
                per_replica_losses_ps * class_weight[1] +\
                per_replica_losses_EQmask * class_weight[2] + \
                per_replica_losses_RFmask * class_weight[3]

            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses,\
            per_replica_losses_rfocc,\
            per_replica_losses_ps,\
            per_replica_losses_EQmask, per_replica_losses_RFmask

    per_replica_losses, \
         per_replica_losses_rfocc,\
        per_replica_losses_ps, per_replica_losses_EQmask, \
        per_replica_losses_RFmask = strategy.run(
            train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )    
    #mean_batch_loss_EQocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
    #    per_replica_losses_eqocc, axis=None ) 
    mean_batch_loss_RFocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_rfocc, axis=None )              
    mean_batch_loss_ps = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_ps, axis=None )
    mean_batch_loss_EQmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_EQmask, axis=None ) 
    mean_batch_loss_RFmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_RFmask, axis=None ) 
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss, mean_batch_loss_RFocc,\
        mean_batch_loss_ps, mean_batch_loss_EQmask, mean_batch_loss_RFmask

@tf.function
def distributed_val_step_RF_fusion_net(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_spec, val_ps, val_EQmask, val_RFmask,
        val_eqocc, val_rfocc,
        loss_estimator, class_weight):
        # estimate validation data 
        val_pred_rfocc, val_pred_ps, val_pred_EQmask, \
            val_pred_RFmask = model([val_trc, val_spec], training=False)
        #per_replica_losses_EQocc = loss_estimator(val_eqocc, val_pred_eqocc)
        per_replica_losses_RFocc = loss_estimator(val_rfocc, val_pred_rfocc)            
        per_replica_losses_ps = tf.reduce_sum(
            loss_estimator(val_ps, val_pred_ps), axis=1)
        per_replica_losses_EQmask = tf.reduce_sum(
            loss_estimator(val_EQmask, val_pred_EQmask), axis=1)
        per_replica_losses_RFmask = tf.reduce_sum(
            loss_estimator(val_RFmask, val_pred_RFmask), axis=1)

        per_replica_losses = \
            per_replica_losses_RFocc * class_weight[0] + \
            per_replica_losses_ps * class_weight[1] +\
            per_replica_losses_EQmask * class_weight[2] + \
            per_replica_losses_RFmask * class_weight[3]

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss


@tf.function
def distributed_train_step_RF_wf_net(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_ps, train_EQmask, train_RFmask,
        train_eqocc, train_rfocc,
        loss_estimator, class_weight):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_rfocc, train_pred_ps,\
                train_pred_EQmask, train_pred_RFmask = model(
                    train_trc, training=True
                )
            #per_replica_losses_eqocc = \
            #        loss_estimator(train_eqocc, train_pred_eqocc)
            per_replica_losses_rfocc = \
                    loss_estimator(train_rfocc, train_pred_rfocc)
            per_replica_losses_ps = \
                tf.reduce_sum(
                    loss_estimator(train_ps, train_pred_ps),
                axis=1)
            per_replica_losses_EQmask = \
                tf.reduce_sum(
                    loss_estimator(train_EQmask, train_pred_EQmask),
                axis=1)
            per_replica_losses_RFmask = \
                tf.reduce_sum(
                    loss_estimator(train_RFmask, train_pred_RFmask),
                axis=1)

            per_replica_losses =\
                per_replica_losses_rfocc * class_weight[0] +\
                per_replica_losses_ps * class_weight[1] +\
                per_replica_losses_EQmask * class_weight[2] + \
                per_replica_losses_RFmask * class_weight[3]

            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses,\
            per_replica_losses_rfocc,\
            per_replica_losses_ps,\
            per_replica_losses_EQmask, per_replica_losses_RFmask

    per_replica_losses, \
         per_replica_losses_rfocc,\
        per_replica_losses_ps, per_replica_losses_EQmask, \
        per_replica_losses_RFmask = strategy.run(
            train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )    
    #mean_batch_loss_EQocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
    #    per_replica_losses_eqocc, axis=None ) 
    mean_batch_loss_RFocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_rfocc, axis=None )              
    mean_batch_loss_ps = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_ps, axis=None )
    mean_batch_loss_EQmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_EQmask, axis=None ) 
    mean_batch_loss_RFmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_RFmask, axis=None ) 
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss, mean_batch_loss_RFocc,\
        mean_batch_loss_ps, mean_batch_loss_EQmask, mean_batch_loss_RFmask

@tf.function
def distributed_val_step_RF_wf_net(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_ps, val_EQmask, val_RFmask,
        val_eqocc, val_rfocc,
        loss_estimator, class_weight):
        # estimate validation data 
        val_pred_rfocc, val_pred_ps, val_pred_EQmask, \
            val_pred_RFmask = model(val_trc, training=False)
        #per_replica_losses_EQocc = loss_estimator(val_eqocc, val_pred_eqocc)
        per_replica_losses_RFocc = loss_estimator(val_rfocc, val_pred_rfocc)            
        per_replica_losses_ps = tf.reduce_sum(
            loss_estimator(val_ps, val_pred_ps), axis=1)
        per_replica_losses_EQmask = tf.reduce_sum(
            loss_estimator(val_EQmask, val_pred_EQmask), axis=1)
        per_replica_losses_RFmask = tf.reduce_sum(
            loss_estimator(val_RFmask, val_pred_RFmask), axis=1)

        per_replica_losses = \
            per_replica_losses_RFocc * class_weight[0] + \
            per_replica_losses_ps * class_weight[1] +\
            per_replica_losses_EQmask * class_weight[2] + \
            per_replica_losses_RFmask * class_weight[3]

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss

@tf.function
def distributed_train_step_RF_fusion_net(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_spec, train_ps, train_EQmask, train_RFmask,
        train_eqocc, train_rfocc,
        loss_estimator, class_weight):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_rfocc, train_pred_eqocc, train_pred_ps,\
                train_pred_EQmask, train_pred_RFmask = model(
                    [train_trc, train_spec], training=True
                )
            #per_replica_losses_eqocc = \
            #        loss_estimator(train_eqocc, train_pred_eqocc)
            per_replica_losses_rfocc = \
                    loss_estimator(train_rfocc, train_pred_rfocc)
            per_replica_losses_ps = \
                tf.reduce_sum(
                    loss_estimator(train_ps, train_pred_ps),
                axis=1)
            per_replica_losses_EQmask = \
                tf.reduce_sum(
                    loss_estimator(train_EQmask, train_pred_EQmask),
                axis=1)
            per_replica_losses_RFmask = \
                tf.reduce_sum(
                    loss_estimator(train_RFmask, train_pred_RFmask),
                axis=1)

            per_replica_losses =\
                per_replica_losses_rfocc * class_weight[0] +\
                per_replica_losses_ps * class_weight[1] +\
                per_replica_losses_EQmask * class_weight[2] + \
                per_replica_losses_RFmask * class_weight[3]

            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses,\
            per_replica_losses_rfocc,\
            per_replica_losses_ps,\
            per_replica_losses_EQmask, per_replica_losses_RFmask

    per_replica_losses, \
         per_replica_losses_rfocc,\
        per_replica_losses_ps, per_replica_losses_EQmask, \
        per_replica_losses_RFmask = strategy.run(
            train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )    
    #mean_batch_loss_EQocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
    #    per_replica_losses_eqocc, axis=None ) 
    mean_batch_loss_RFocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_rfocc, axis=None )              
    mean_batch_loss_ps = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_ps, axis=None )
    mean_batch_loss_EQmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_EQmask, axis=None ) 
    mean_batch_loss_RFmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_RFmask, axis=None ) 
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss, mean_batch_loss_RFocc,\
        mean_batch_loss_ps, mean_batch_loss_EQmask, mean_batch_loss_RFmask

@tf.function
def distributed_val_step_RF_fusion_net(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_spec, val_ps, val_EQmask, val_RFmask,
        val_eqocc, val_rfocc,
        loss_estimator, class_weight):
        # estimate validation data 
        val_pred_rfocc, val_pred_occ, val_pred_ps, val_pred_EQmask, \
            val_pred_RFmask = model([val_trc, val_spec], training=False)
        #per_replica_losses_EQocc = loss_estimator(val_eqocc, val_pred_eqocc)
        per_replica_losses_RFocc = loss_estimator(val_rfocc, val_pred_rfocc)            
        per_replica_losses_ps = tf.reduce_sum(
            loss_estimator(val_ps, val_pred_ps), axis=1)
        per_replica_losses_EQmask = tf.reduce_sum(
            loss_estimator(val_EQmask, val_pred_EQmask), axis=1)
        per_replica_losses_RFmask = tf.reduce_sum(
            loss_estimator(val_RFmask, val_pred_RFmask), axis=1)

        per_replica_losses = \
            per_replica_losses_RFocc * class_weight[0] + \
            per_replica_losses_ps * class_weight[1] +\
            per_replica_losses_EQmask * class_weight[2] + \
            per_replica_losses_RFmask * class_weight[3]

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss

@tf.function
def distributed_train_step_RF_fusion_net_light(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_spec, train_ps, train_EQmask, train_RFmask,
        train_eqocc, train_rfocc,
        loss_estimator, class_weight):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_rfocc, train_pred_eqocc, train_pred_ps,\
                train_pred_EQmask, train_pred_RFmask = model(
                    [train_trc, train_spec], training=True
                )
            per_replica_losses_eqocc = \
                    loss_estimator(train_eqocc, train_pred_eqocc)
            per_replica_losses_rfocc = \
                    loss_estimator(train_rfocc, train_pred_rfocc)
            per_replica_losses_ps = \
                tf.reduce_sum(
                    loss_estimator(train_ps, train_pred_ps),
                axis=1)
            per_replica_losses_EQmask = \
                tf.reduce_sum(
                    loss_estimator(train_EQmask, train_pred_EQmask),
                axis=1)
            per_replica_losses_RFmask = \
                tf.reduce_sum(
                    loss_estimator(train_RFmask, train_pred_RFmask),
                axis=1)

            per_replica_losses =\
                per_replica_losses_rfocc * class_weight[0] +\
                per_replica_losses_eqocc * class_weight[1] +\
                per_replica_losses_ps * class_weight[2] +\
                per_replica_losses_EQmask * class_weight[3] + \
                per_replica_losses_RFmask * class_weight[4]

            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses,\
            per_replica_losses_rfocc,\
            per_replica_losses_eqocc,\
            per_replica_losses_ps,\
            per_replica_losses_EQmask, per_replica_losses_RFmask

    per_replica_losses, \
        per_replica_losses_rfocc, \
        per_replica_losses_eqocc, \
        per_replica_losses_ps, per_replica_losses_EQmask, \
        per_replica_losses_RFmask = strategy.run(
            train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )    
    mean_batch_loss_EQocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_eqocc, axis=None ) 
    mean_batch_loss_RFocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_rfocc, axis=None )              
    mean_batch_loss_ps = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_ps, axis=None )
    mean_batch_loss_EQmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_EQmask, axis=None ) 
    mean_batch_loss_RFmask = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_RFmask, axis=None ) 
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss, mean_batch_loss_RFocc,mean_batch_loss_EQocc,\
        mean_batch_loss_ps, mean_batch_loss_EQmask, mean_batch_loss_RFmask

@tf.function
def distributed_val_step_RF_fusion_net_light(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_spec, val_ps, val_EQmask, val_RFmask,
        val_eqocc, val_rfocc,
        loss_estimator, class_weight):
        # estimate validation data 
        val_pred_rfocc, val_pred_eqocc,\
            val_pred_ps, val_pred_EQmask, val_pred_RFmask = \
                model([val_trc, val_spec], training=False)
        per_replica_losses_EQocc = loss_estimator(val_eqocc, val_pred_eqocc)
        per_replica_losses_RFocc = loss_estimator(val_rfocc, val_pred_rfocc)            
        per_replica_losses_ps = tf.reduce_sum(
            loss_estimator(val_ps, val_pred_ps), axis=1)
        per_replica_losses_EQmask = tf.reduce_sum(
            loss_estimator(val_EQmask, val_pred_EQmask), axis=1)
        per_replica_losses_RFmask = tf.reduce_sum(
            loss_estimator(val_RFmask, val_pred_RFmask), axis=1)

        per_replica_losses = \
            per_replica_losses_RFocc * class_weight[0] + \
            per_replica_losses_EQocc * class_weight[1] + \
            per_replica_losses_ps * class_weight[2] +\
            per_replica_losses_EQmask * class_weight[3] + \
            per_replica_losses_RFmask * class_weight[4]

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss

@tf.function
def distributed_train_step_RF_fusion_net_light_v2(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_spec, train_ps, train_EQmask, train_RFmask,
        train_eqocc, train_rfocc,
        loss_estimator, class_weight):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_rfocc, train_pred_eqocc, train_pred_ps,\
                train_pred_EQmask, train_pred_RFmask = model(
                    [train_trc, train_spec], training=True
                )
            per_replica_losses_eqocc = \
                    loss_estimator(train_eqocc, train_pred_eqocc)
            per_replica_losses_rfocc = \
                    loss_estimator(train_rfocc, train_pred_rfocc)


            per_replica_losses =\
                per_replica_losses_rfocc * class_weight[0] +\
                per_replica_losses_eqocc * class_weight[1]

            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses,\
            per_replica_losses_rfocc,\
            per_replica_losses_eqocc

    per_replica_losses, \
        per_replica_losses_rfocc, \
        per_replica_losses_eqocc = strategy.run(
            train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )    
    mean_batch_loss_EQocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_eqocc, axis=None ) 
    mean_batch_loss_RFocc = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses_rfocc, axis=None )              
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss, mean_batch_loss_RFocc,mean_batch_loss_EQocc

@tf.function
def distributed_val_step_RF_fusion_net_light_v2(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_spec, val_ps, val_EQmask, val_RFmask,
        val_eqocc, val_rfocc,
        loss_estimator, class_weight):
        # estimate validation data 
        val_pred_rfocc, val_pred_eqocc,\
            val_pred_ps, val_pred_EQmask, val_pred_RFmask = \
                model([val_trc, val_spec], training=False)
        per_replica_losses_EQocc = loss_estimator(val_eqocc, val_pred_eqocc)
        per_replica_losses_RFocc = loss_estimator(val_rfocc, val_pred_rfocc)            

        per_replica_losses = \
            per_replica_losses_RFocc * class_weight[0] + \
            per_replica_losses_EQocc * class_weight[1]

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss

@tf.function
def distributed_train_step_RF_fusion_1task(strategy, train_args):
    def train_step_gpus(model, opt, global_batch_size,
        train_trc, train_spec, train_RFmask, 
        loss_estimator):
        # compute loss for gradient descent
        with tf.GradientTape() as tape:
            # make predictions and estimate loss
            train_pred_RFmask = model([train_trc, train_spec], training=True)
            per_replica_losses_RFmask = \
                loss_estimator(train_RFmask, train_pred_RFmask)

            per_replica_losses = per_replica_losses_RFmask
            grad_loss = tf.reduce_sum(per_replica_losses)/global_batch_size
        # calculate the gradients and update the weights
        grad = tape.gradient(grad_loss, model.trainable_variables)
        opt.apply_gradients(zip(grad, model.trainable_variables))
        return per_replica_losses

    per_replica_losses = strategy.run(train_step_gpus, args=train_args)

    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None )                 
    #train_loss_avg.update_state(mean_loss)
    return mean_batch_loss

@tf.function
def distributed_val_step_RF_fusion_1task(strategy, val_args):
    def val_step_gpus(
        model, global_batch_size,
        val_trc, val_spec, val_RFmask,
        loss_estimator):
        # estimate validation data 
        val_pred_RFmask = model([val_trc, val_spec], training=False)
        per_replica_losses_RFmask = loss_estimator(val_RFmask, val_pred_RFmask)
        per_replica_losses = per_replica_losses_RFmask

        return per_replica_losses
    per_replica_losses = strategy.run(val_step_gpus, args=val_args)
    mean_batch_loss = strategy.reduce(tf.distribute.ReduceOp.SUM, 
        per_replica_losses, axis=None)
    return mean_batch_loss

if __name__=='__main__':
    pass